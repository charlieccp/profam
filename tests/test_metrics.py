...
def test_mixed_dataset_batch():
    ...
    # Introduce errors
    logits[0, 1, 3] = -1000  # incorrect in ds1, first sequence
    logits[1, 1, 3] = -1000  # incorrect in ds2, first sequence
    ...
    # Verify dataset-specific metrics with errors
    assert torch.isclose(
        acc_metrics["ds1"], torch.tensor(5 / 6)
    )  # 5 correct out of 6 tokens
    assert torch.isclose(
        acc_metrics["ds2"], torch.tensor(2 / 3)
    )  # 1 correct out of 2 tokens
    assert torch.isclose(
        acc_metrics["ds1_last_sequence"], torch.tensor(1.0)
    )  # last sequence still perfect
    assert torch.isclose(
        acc_metrics["ds2_last_sequence"], torch.tensor(2 / 3)
    )  # 1 error in only sequence
